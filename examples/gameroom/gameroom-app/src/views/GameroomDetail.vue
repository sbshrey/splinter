...
<template>
  <div class="supplychain-detail-container">
    ...
    <div class="supplychain-information">
      <h2 class="supplychain-name">{{ supplychain.alias }}</h2>
      <span> {{ gemeroomMembers }} </span>
    </div>
        <div v-if="supplychain.status === 'Active'" class="data-container">
          <div class="header">
          <div class="tab-buttons">
          <button class="tab-button"
                  @click="selectTab(1)"
                  :disabled="supplychain.status !== 'Active'"
        ...
        <button  :disabled="supplychain.status !== 'Active'" class="btn-action right" @click="showNewGameModal()">
    ...
    <div class="data-container" v-else>
      <loading message="Please wait while the supplychain is being set up" />
    </div>
  </div>
</template>
...
import { Supplychain, Member, Game } from '@/store/models';
...
@Component({
  ...
  export default class SupplychainDetails extends Vue {
      gameNameFilter = '';
      currentTab = 1;
      newGameName = '';
      displayModal = false;
      ...
      async beforeRouteEnter(to: any, from: any , next: any) {
        store.commit('pageLoading/setPageLoading', 'Loading supplychain');
        try {
          await store.dispatch('selectedSupplychain/updateSelectedSupplychain', to.params.id);
      ...
      get supplychain(): Supplychain {
         return this.$store.getters['selectedSupplychain/getSupplychain'];
      }
      ...
      get gemeroomMembers() {
        if (this.supplychain.members) {
          const organizations = this.supplychain.members.map((member) => member.organization);
...
<style lang="scss" scoped>
  @import '@/scss/components/_supplychain-details.scss';
</style>
